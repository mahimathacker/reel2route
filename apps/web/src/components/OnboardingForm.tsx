...
export function OnboardingForm({ initialPreferences, error, onSubmit }: Props) {
  ...
  return <div className="create-view">
    ...
    <form className="planner-card" onSubmit={submit}>
      ...
      <div className="form-grid"><label className="input-group"><span>Travelling from</span><input required minLength={2} placeholder="Mumbai, India" aria-describedby="origin-hint" value={preferences.origin} onChange={(event) => setPreferences({...preferences,origin:event.target.value})} /><small id="origin-hint">Use City, Country for a more reliable route estimate.</small></label><label className="input-group"><span>Trip length</span><span className="number-input"><input type="number" min={1} max={14} value={preferences.days} onChange={(event) => setPreferences({...preferences,days:Number(event.target.value)})} /><small>days</small></span></label></div>
